...
module.exports.samplePayment = function(projectId, urlPrefix, isAuth, options, callback) {
    db.findOne('project', {
        _id: projectId
        ...
        var method = isAuth && typeof bankObjects[banks[project.bank].type].sampleAuth === 'function' ? 'sampleAuth' : 'samplePayment';
...
function serveBanklink(req, res) {
    ...
    project.validateClient(function(err, data) {
        ...
        if (err) {
            ...
            return logPayment(project, 'ERROR', req, res, {
                errors: errors,
                warnings: warnings
...
function autoPay(req, res, id, payment) {
    ...
    var ap = (req.body.PANGALINK_AUTOPAY || '').toString().trim().toLowerCase(),
        options = {
            pay: ap === 'accept' ? 'pay' : '',
    ...
    makePayment(id, options, function(err, data) {
        if (err) {
...
function makePayment(id, options, callback) {
    db.findOne('payment', {
        _id: id
    }, function(err, record) {
        if (err) {
